...
resource "null_resource" "sqlcl-create-usr" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
                ...
                wget https://github.com/marancibia/marancibia.github.io/raw/refs/heads/main/adb/AskOracle/f101.sql -P ./scripts/
...
resource "local_file" "this2" {
  content  = templatefile("./scripts/install-apex-app.sql.tmpl", { llm_region = var.llm_region, schema_name = local.effective_schema_name })
