...
6. On the Oracle Machine Learning home page, click **Notebooks** in the Quick Actions menu.
    ...
    ![Oracle Machine Learning home page with click on the Notebooks icon](images/oml-notebooks-homepage.png " ")

## Task 2: Get familiar with OML Notebooks

<if type="freetier">
1. [**CLICK HERE** to download a ZIP file with all the notebooks we will use for these Labs (in JSON format)](https://objectstorage.us-ashburn-1.oraclecloud.com/p/nF2LwFUfXmY_uID0nAwz1vccWvT4uhLcMgXjYcFW0IH_9rF-wUAEESCkiZio40_0/n/adwc4pm/b/OML_Data/o/oml4py_labs_freetier_23.zip?download=1) which contain the notebooks for the Labs.  Save it to your local machine and __extract them__ to a memorable place.
  </if>
<if type="livelabs">
1. [**CLICK HERE** to download a ZIP file with all the notebooks we will use for these Labs (in JSON format)](https://objectstorage.us-ashburn-1.oraclecloud.com/p/62bzAesJOPufvH5nHFV-w72UJYbACvkcjzkE1RPPfmimMOTGDFaiNjqoEjJZqBPM/n/adwc4pm/b/OML_Data/o/oml4py_labs_livelabs_23.zip?download=1) which contain the notebooks for the Labs.  Save it to your local machine and __extract them__ to a memorable place.
  </if>
...
> **NOTE:** If you have problems with downloading and extracting the ZIP file, please [**CLICK HERE** to download the "Lab 1: Get Started with OML4Py on Autonomous Database" notebook in JSON format](./../notebooks/lab1_get_started.json?download=1) which contains the notebook version of Lab1, save it to your local machine and import it as illustrated below.
  ...
  ![Oracle Machine Learning Notebooks import screen capture](images/click-on-import-notebook.png " ")

 - Select all the extracted notebooks at once and click on **Open**.
 <if type="freetier">   ![Select the 7 JSON files on your machine and click Open](images/open-lab-files-ft.png " ")</if>
 <if type="livelabs">   ![Select the 6 JSON files on your machine and click Open](images/open-lab-files-ll.png " ") </if>
...
For example, it displays the status and the number of users connected to the notebook. It also contains a menu item for keyboard shortcuts and options to show or hide the markdown editor and paragraph output. Additional settings are shown in the illustration below.
    ![Explanation of the Notebook toolbar options and icons](images/notebook_toolbar.png " ")

## Task 3: Getting started with OML Notebooks and interpreters

1. OML Notebooks contain a default list of interpreter bindings, used to run scripts against the Autonomous Database that it is linked to.  For this lab, we will set the interpreter binding to "xxxx_medium", where xxxx is the database name.
...
Click the interpreter bindings icon (![](images/interpreter_bindings_icon_small.png "")) in the upper right-corner of the notebook (the gear) to view the list of available interpreter bindings.
  ![Oracle Machine Learning Notebooks Interpreter Bindings screen capture](images/interpreter_bindings.png " ")
...
Another option is to just scroll down and read the pre-recorded results contained in the notebooks themselves.
 ...
 ![Click run all paragraphs in the Introduction section of the notebook Lab1](images/click-run-all-lab1.png " ")
...
1. Labs 2 to 6 of this workshop use tables and views that need to be created upfront.  To create these tables and views, we will open the Notebook "Lab 1a: Run Me First - OML4Py table creation and grants".
...
2. Go back to the main Notebooks listing by clicking on the "hamburger" menu (the three lines) on the upper left of the screen, and then select **Notebooks**.
 ...
 ![Oracle Machine Learning Notebooks menu by clicking the three-lines menu on the top left of the screen](images/go-back-to-notebooks.png " ")

3. Click the **notebook name** to view it.
   ![Open Lab 1a Run Me First notebook](images/click-on-lab1a-ft.png " ")

 OML Notebooks will load a session and make the notebook available for editing.

4. Click the **Run all paragraphs** (![](images/run-all-paragraphs.png =20x*)) icon, and then click **OK** to confirm.
  ![On Introduction screen of Notebook click on run all button](images/click-run-all.png " ")

5. Wait until all the paragraphs have finished running and you see your current time in the last paragraph after the message `"PL/SQL procedure successfully completed"`.

  ![The last paragraph with the current timestamp in the notebook](images/last-paragraph.png " ")
